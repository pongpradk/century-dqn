...
class DQNAgent:
    ...
    def save_experience(self, state, action, reward, next_state, terminal):
        """ Compute TD error and save prioritized experience, prioritizing golem purchases """
        ...
        # 🔹 Prioritize golem purchases by increasing their priority
        GOLEM_ACTIONS = [10, 11, 12, 13, 14]  # Update these indices based on your actual environment
        if action in GOLEM_ACTIONS:
...
if __name__ == '__main__':
    ...
    if continue_training:
        dqn_agent.load_agent('random')  # Load saved agent
        metadata = load_training_metadata('training_metadataRandom.json')
    else:
    ...
    try:
        for ep in range(metadata['episode'], num_episodes):
            ...
            # if ep % 1000 == 0 and dqn_agent.epsilon > dqn_agent.epsilon_min:
            #     dqn_agent.epsilon = min(0.2, dqn_agent.epsilon * 1.1)  # Small exploration boost but never exceeds 0.2
        ...
        dqn_agent.save_agent('random')
        save_training_metadata('training_metadataRandom.json', {
            'episode': ep + 1,
            'time_step': time_step,
            'rewards': rewards,
        ...
        try:
            with open('rewardsRandom.txt', 'r') as f:
        ...
        with open('rewardsRandom.txt', 'w') as f:  # Overwrite with full updated list
            f.write(json.dumps(updated_rewards))
        print("Rewards of the training saved in 'rewardsRandom.txt'")

        # ✅ Append new epsilon values to 'epsilon_values.txt' (keeping previous records)
        try:
            with open('epsilon_valuesRandom.txt', 'r') as f:
        ...
        with open('epsilon_valuesRandom.txt', 'w') as f:  # Overwrite with full updated list
            f.write(json.dumps(updated_epsilon_values))
        print("Epsilon values of the training saved in 'epsilon_valuesRandom.txt'")
