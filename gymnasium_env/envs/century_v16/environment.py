...
class CenturyGolemEnv(gym.Env):
    ...
    def _handle_rest(self):
        # Update the merchant_cards_status array based on the current player
        if self.current_player.player_id == 1:
            # Convert P1's M1/M2 unplayable cards to playable (indices 0-1)
            self.merchant_cards_status[0] = MerchantCardStatus.P1_PLAYABLE.value
            self.merchant_cards_status[1] = MerchantCardStatus.P1_PLAYABLE.value
            
            # Convert P1's regular unplayable cards to playable
            for i in range(4, 47):
                if self.merchant_cards_status[i] == MerchantCardStatus.REG_P1_UNPLAYABLE.value:
                    self.merchant_cards_status[i] = MerchantCardStatus.REG_P1_PLAYABLE.value
        else:
            # Convert P2's M1/M2 unplayable cards to playable (indices 2-3)
            self.merchant_cards_status[2] = MerchantCardStatus.P2_PLAYABLE.value
            self.merchant_cards_status[3] = MerchantCardStatus.P2_PLAYABLE.value
            
            # Convert P2's regular unplayable cards to playable
            for i in range(4, 47):
                if self.merchant_cards_status[i] == MerchantCardStatus.REG_P2_UNPLAYABLE.value:
                    self.merchant_cards_status[i] = MerchantCardStatus.REG_P2_PLAYABLE.value
    ...
    def _handle_get_merchant_card(self, action):
        ...
        if self.merchant_deck[card_id] in self.merchant_market:
            ...
            # M3-M45 cards use the regular card status
            card_index = card_id + 1  # +1 to account for the 4 starter card statuses
            
            if self.current_player.player_id == 1:
                self.merchant_cards_status[card_index] = MerchantCardStatus.REG_P1_PLAYABLE.value
            else:
                self.merchant_cards_status[card_index] = MerchantCardStatus.REG_P2_PLAYABLE.value
            ...
            owned_count = 0
            if self.current_player.player_id == 1:
                # Count starter cards (always 2) plus regular cards owned by P1
                owned_count = 2 + sum(1 for i in range(4, 47) 
                ...
                # Count starter cards (always 2) plus regular cards owned by P2
                owned_count = 2 + sum(1 for i in range(4, 47) 
    ...
    def _get_card_status_index(self, card_id, player_id):
        """Helper method to get the correct index in merchant_cards_status for a card"""
        if card_id == 1:  # M1
            return 0 if player_id == 1 else 2
        elif card_id == 2:  # M2
            return 1 if player_id == 1 else 3
        else:  # M3-M45
            return card_id + 1  # Add 1 to account for the 4 starter card statuses
            
    def _is_starter_card(self, card_id):
        """Helper method to check if a card is a starter card (M1 or M2)"""
    ...
    def _handle_crystal_card(self, card, card_index):
        ...
        # Set card as unplayable for the current player
        card_id = card_index + 1
        status_index = self._get_card_status_index(card_id, self.current_player.player_id)
        
        if self._is_starter_card(card_id):
            # For M1/M2
            self.merchant_cards_status[status_index] = MerchantCardStatus.P1_UNPLAYABLE.value if self.current_player.player_id == 1 else MerchantCardStatus.P2_UNPLAYABLE.value
        else:
        ...
        return (GAME_CONSTANTS['CRYSTAL_VALUES']['yellow'] * card.gain.get('yellow', 0)) + (GAME_CONSTANTS['CRYSTAL_VALUES']['green'] * card.gain.get('green', 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['blue'] * card.gain.get('blue', 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['pink'] * card.gain.get('pink', 0))
    ...
    def _handle_trade_card(self, card, card_index): 
        ...
        if all(self.current_player.caravan[crystal] >= amount 
            ...
            loss = (GAME_CONSTANTS['CRYSTAL_VALUES']['yellow'] * card.cost.get("yellow", 0)) + (GAME_CONSTANTS['CRYSTAL_VALUES']['green'] * card.cost.get("green", 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['blue'] * card.cost.get("blue", 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['pink'] * card.cost.get('pink', 0))
            gain = (GAME_CONSTANTS['CRYSTAL_VALUES']['yellow'] * card.gain.get("yellow", 0)) + (GAME_CONSTANTS['CRYSTAL_VALUES']['green'] * card.gain.get("green", 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['blue'] * card.gain.get("blue", 0) + GAME_CONSTANTS['CRYSTAL_VALUES']['pink'] * card.gain.get('pink', 0))
            return (gain - loss)
    ...
    def _handle_upgrade_card(self, card, card_index):
        ...
        # Greedy upgrade from lowest to highest: yellow -> green -> blue -> pink
        while upgrade_points > 0:
            # Try to upgrade yellow -> green first
    ...
    def _handle_use_merchant_card(self, action):
        ...
        # Get the correct status index for this card
        status_index = self._get_card_status_index(card_id, self.current_player.player_id)
        
        # Check if card is playable for the current player
        is_playable = False
        
        if self._is_starter_card(card_id):
            # For M1/M2
            if self.current_player.player_id == 1 and self.merchant_cards_status[status_index] == MerchantCardStatus.P1_PLAYABLE.value:
                is_playable = True
            elif self.current_player.player_id == 2 and self.merchant_cards_status[status_index] == MerchantCardStatus.P2_PLAYABLE.value:
                is_playable = True
        else:
            # For regular cards M3-M45
            if self.current_player.player_id == 1 and self.merchant_cards_status[status_index] == MerchantCardStatus.REG_P1_PLAYABLE.value:
                is_playable = True
            elif self.current_player.player_id == 2 and self.merchant_cards_status[status_index] == MerchantCardStatus.REG_P2_PLAYABLE.value:
                is_playable = True
            
        if is_playable and card:
        ...
        # Strategic crystal management reward calculation remains the same
        caravan = self.current_player.caravan
    ...
    def __init__(self, render_mode=None):
        ...
        # Open information
        crystal_types = ["yellow", "green", "blue", "pink"]  # Add more types here as needed
        ...
        self.observation_space = spaces.Dict({
            ...
            # Expanded merchant cards status: 
            # First 4 elements: Player-specific M1/M2 (2 for each player)
            # Remaining 43 elements: Status for cards M3-M45
        ...
        # Initialize centralized merchant card status array with 47 elements:
        # [0]: Player 1's M1 status (0=unplayable, 1=playable)
        # [1]: Player 1's M2 status (0=unplayable, 1=playable)
        # [2]: Player 2's M1 status (0=unplayable, 1=playable)
        # [3]: Player 2's M2 status (0=unplayable, 1=playable)
        ...
        # Set initial status - both players have M1 and M2 as playable
    ...
    def _get_obs(self, player):
        ...
        # Ensure merchant card status is within proper ranges
        # First 4 elements have range [0,1], rest have range [0,4]
    ...
    def reset(self, seed=None, options=None):
        if self.render_mode == "text":
            print("Century: Golem Edition | Version 16\n")
        ...
        # First reset all cards M3-M45 to unowned
        for i in range(4, 47):
            self.merchant_cards_status[i] = MerchantCardStatus.UNOWNED.value
            
        # Then initialize starter cards for both players
        # Default is all starter cards are playable at game start
        self.merchant_cards_status[0] = MerchantCardStatus.P1_PLAYABLE.value  # P1's M1
        self.merchant_cards_status[1] = MerchantCardStatus.P1_PLAYABLE.value  # P1's M2
        self.merchant_cards_status[2] = MerchantCardStatus.P2_PLAYABLE.value  # P2's M1
        self.merchant_cards_status[3] = MerchantCardStatus.P2_PLAYABLE.value  # P2's M2
    ...
    def _get_valid_actions(self, player):
        ...
        # Check if any of player's starter cards (M1/M2) are unplayable
        if (player.player_id == 1 and 
            (self.merchant_cards_status[0] == MerchantCardStatus.P1_UNPLAYABLE.value or 
             self.merchant_cards_status[1] == MerchantCardStatus.P1_UNPLAYABLE.value)):
            valid_actions[Actions.rest.value] = 1
        elif (player.player_id == 2 and 
              (self.merchant_cards_status[2] == MerchantCardStatus.P2_UNPLAYABLE.value or 
               self.merchant_cards_status[3] == MerchantCardStatus.P2_UNPLAYABLE.value)):
            valid_actions[Actions.rest.value] = 1
            
        # Check if any of player's regular cards (M3-M45) are unplayable
        if valid_actions[Actions.rest.value] == 0:  # Skip if rest is already valid
            for i in range(4, 47):
                if (player.player_id == 1 and self.merchant_cards_status[i] == MerchantCardStatus.REG_P1_UNPLAYABLE.value) or \
                   (player.player_id == 2 and self.merchant_cards_status[i] == MerchantCardStatus.REG_P2_UNPLAYABLE.value):
                    valid_actions[Actions.rest.value] = 1
                    break
        ...
        # Use merchant card actions for M1 and M2
        if player.player_id == 1:
            # M1 (index 0 for P1)
            if self.merchant_cards_status[0] == MerchantCardStatus.P1_PLAYABLE.value:
                card = self.merchant_deck.get(1)
                # Crystal and upgrade cards are always valid
                if card.card_type in ["crystal", "upgrade"]:
                    valid_actions[Actions.useM1.value] = 1
                # For trade cards, check if enough crystals
                elif card.card_type == "trade" and all(player.caravan[crystal] >= amount for crystal, amount in card.cost.items()):
                    valid_actions[Actions.useM1.value] = 1
                    
            # M2 (index 1 for P1)
            if self.merchant_cards_status[1] == MerchantCardStatus.P1_PLAYABLE.value:
                card = self.merchant_deck.get(2)
                # Crystal cards are always valid
                if card.card_type == "crystal":
                    valid_actions[Actions.useM2.value] = 1
                # For trade cards, check if enough crystals
                elif card.card_type == "trade" and all(player.caravan[crystal] >= amount for crystal, amount in card.cost.items()):
                    valid_actions[Actions.useM2.value] = 1
                # For upgrade cards, check if there are crystals to upgrade
                elif card.card_type == "upgrade" and (player.caravan["yellow"] > 0 or player.caravan["green"] > 0 or player.caravan["blue"] > 0):
                    valid_actions[Actions.useM2.value] = 1
        else:
            # M1 (index 2 for P2)
            if self.merchant_cards_status[2] == MerchantCardStatus.P2_PLAYABLE.value:
                card = self.merchant_deck.get(1)
                # Crystal and upgrade cards are always valid
                if card.card_type in ["crystal", "upgrade"]:
                    valid_actions[Actions.useM1.value] = 1
            ...
            # M2 (index 3 for P2)
            if self.merchant_cards_status[3] == MerchantCardStatus.P2_PLAYABLE.value:
                card = self.merchant_deck.get(2)
                # Crystal cards are always valid
                if card.card_type == "crystal":
                    valid_actions[Actions.useM2.value] = 1
        ...
        # Use merchant card actions for M3-M45
        for i in range(Actions.useM3.value, Actions.useM45.value + 1):
            card_id = i - Actions.useM1.value + 1
            card_index = card_id + 1  # Fixed: +1 instead of +2
            
            is_playable = False
            if player.player_id == 1 and self.merchant_cards_status[card_index] == MerchantCardStatus.REG_P1_PLAYABLE.value:
                is_playable = True
            elif player.player_id == 2 and self.merchant_cards_status[card_index] == MerchantCardStatus.REG_P2_PLAYABLE.value:
                is_playable = True
            ...
            if is_playable:
                ...
                if card.card_type == "crystal":
                    ...
                    if player.caravan["yellow"] > 0 or player.caravan["green"] > 0 or player.caravan["blue"] > 0:
                        valid_actions[i] = 1

        # Get golem card actions
        for i in range(Actions.getG1.value, Actions.getG36.value + 1):
    ...
    def render(self):
        if self.render_mode == "text":               
            ...
            print("")
            print(f"DQN Agent")
            print(f"Y:{self.player1.caravan['yellow']} | G:{self.player1.caravan['green']} | B:{self.player1.caravan['blue']} | P:{self.player1.caravan['pink']}")
            
            # Display P1's merchant cards
            # First M1 and M2 (special case)
            status_map_starter = {
                MerchantCardStatus.P1_UNPLAYABLE.value: "unplayable",
                MerchantCardStatus.P1_PLAYABLE.value: "playable"
            }
            for i in range(2):  # P1's M1 and M2 (indices 0 and 1)
                card = self.merchant_deck.get(i + 1)  # Card IDs are 1-indexed
                if card:
                    print(f"M{i+1}-{card.name}: {status_map_starter[self.merchant_cards_status[i]]}")
                    
            # Then regular merchant cards M3-M45
            status_map_regular = {
                MerchantCardStatus.REG_P1_UNPLAYABLE.value: "unplayable",
                MerchantCardStatus.REG_P1_PLAYABLE.value: "playable"
            }
            for i in range(4, 47):
                # Convert status index back to card_id
                # In _get_card_status_index, we do: index = card_id + 1
                # So here we do: card_id = index - 1
                card_id = i - 1
                if self.merchant_cards_status[i] in [MerchantCardStatus.REG_P1_UNPLAYABLE.value, MerchantCardStatus.REG_P1_PLAYABLE.value]:
                    card = self.merchant_deck.get(card_id)
                    if card:
                        print(f"M{card_id}-{card.name}: {status_map_regular[self.merchant_cards_status[i]]}")
                    
            print(f"GC: {self.player1.golem_count}")
            print(f"P: {self.player1.points}")
            print("")
            print(f"P{self.player2.player_id}")
            print(f"Y:{self.player2.caravan['yellow']} | G:{self.player2.caravan['green']} | B:{self.player2.caravan['blue']} | P:{self.player2.caravan['pink']}")
            
            # Display P2's merchant cards
            # First M1 and M2 (special case)
            status_map_starter = {
                MerchantCardStatus.P2_UNPLAYABLE.value: "unplayable",
                MerchantCardStatus.P2_PLAYABLE.value: "playable"
            }
            for i in range(2, 4):  # P2's M1 and M2 (indices 2 and 3)
                card_id = i - 1 if i == 2 else i - 2  # Convert to card ID (1 or 2)
                card = self.merchant_deck.get(card_id)
                if card:
                    print(f"M{card_id}-{card.name}: {status_map_starter[self.merchant_cards_status[i]]}")
                    
            # Then regular merchant cards M3-M45
            status_map_regular = {
                MerchantCardStatus.REG_P2_UNPLAYABLE.value: "unplayable",
                MerchantCardStatus.REG_P2_PLAYABLE.value: "playable"
            }
            for i in range(4, 47):
                # Convert status index back to card_id
                # In _get_card_status_index, we do: index = card_id + 1
                # So here we do: card_id = index - 1
                card_id = i - 1
                if self.merchant_cards_status[i] in [MerchantCardStatus.REG_P2_UNPLAYABLE.value, MerchantCardStatus.REG_P2_PLAYABLE.value]:
                    card = self.merchant_deck.get(card_id)
                    if card:
                        print(f"M{card_id}-{card.name}: {status_map_regular[self.merchant_cards_status[i]]}")
