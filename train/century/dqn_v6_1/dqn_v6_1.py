...
class DQNAgent:
    ...
    def save_experience(self, state, action, reward, next_state, terminal):
        ...
        if action >= 1 and action <= 23:
            ...
            # Start reducing reward after 5 additional cards (7 total including starting M1, M2)
            if owned_cards > 5:
                # Exponential penalty factor that grows as more cards are acquired
                penalty_factor = 1.0 - min(0.8, 0.2 * (owned_cards - 5))
    ...
    def pick_epsilon_greedy_action(self, state, info):
            ...
            with torch.no_grad():
                ...
                if owned_cards > 5: # Adjusted threshold
                    # Stronger penalty as agent acquires more cards
                    penalty = (owned_cards - 5) * 0.5 # Adjusted penalty calculation
...
if __name__ == '__main__':
    ...
    config_dict = {
        'episodes': args.episodes if args.episodes is not None else 19000,
