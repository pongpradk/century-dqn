...
class DQNAgent:
    ...
    def save_checkpoint(self, episode, checkpoint_dir="checkpoints"):
        ...
        # Ensure epsilon_values exists before saving
        if not hasattr(self, 'epsilon_values'):
            self.epsilon_values = []
...
def train_dqn(config, num_episodes, checkpoint_path=None):
    """
    Train a DQN agent using self-play in generations.
    
    Training progresses in generations:
    1. First generation: Train against RandomAgent
    2. Subsequent generations: Train against model from previous generation
    ...
    num_generations = 5
    episodes_per_generation = num_episodes // num_generations
    print(f"Training with {num_generations} generations, {episodes_per_generation} episodes per generation")
    
    # Start with RandomAgent for first generation
    current_opponent = RandomAgent(action_size)
    ...
    if checkpoint_path:
        ...
        # Determine the current generation based on the start_episode
        current_gen = start_episode // episodes_per_generation
        
        # If not in the first generation, load the appropriate model as the opponent
        if current_gen > 0:
            # Use the generation boundary as the model to load
            prev_gen_episode = current_gen * episodes_per_generation
            model_path = os.path.join("models", f"trained_model_{prev_gen_episode}.pt")
            if os.path.exists(model_path):
                current_opponent = ModelAgent(model_path, state_size, action_size)
                opponent_type = f"ModelAgent from episode {prev_gen_episode}"
            else:
                print(f"Warning: Expected model at {model_path} but not found. Using RandomAgent.")
                current_opponent = RandomAgent(action_size)
                opponent_type = "RandomAgent (fallback)"

    try:
        for ep in range(start_episode, num_episodes):
            # Check if we need to switch to a new generation
            generation = ep // episodes_per_generation
            generation_start_ep = generation * episodes_per_generation
            
            # If we're at the start of a new generation (except gen 0), load the previous model
            if ep > 0 and ep % episodes_per_generation == 0:
                # Use the generation boundary episode (e.g., 200) instead of the last episode of previous gen (e.g., 199)
                prev_gen_model_ep = ep  # This is the beginning of current generation, which is also end of previous gen
                model_path = os.path.join("models", f"trained_model_{prev_gen_model_ep}.pt")
                
                print(f"\n{'='*80}")
                print(f"GENERATION CHANGE: Starting generation {generation+1} at episode {ep+1}")
                print(f"Loading model from episode {prev_gen_model_ep} as the new opponent")
                print(f"{'='*80}\n")
                
                # Small delay to ensure model is fully saved
                time.sleep(2)
                
                # Load the model as the new opponent
                current_opponent = ModelAgent(model_path, state_size, action_size)
                opponent_type = f"ModelAgent from episode {prev_gen_model_ep}"
            
            # Print current generation info every 10 episodes
            if ep % 10 == 0:
                print(f"\nTraining on EPISODE {ep+1} of generation {generation+1}/{num_generations}")
                print(f"Current opponent: {opponent_type}")
            ...
            print(f'Training on EPISODE {ep+1} with epsilon {dqn_agent.epsilon:.4f}')
            start = time.time()
            ...
            for t in range(num_timesteps):
                ...
                if info['current_player'] == 0:
                    ...
                    if not terminal and info['current_player'] == 1:
                        opponent_action = current_opponent.pick_action(next_state, info)
                    ...
                    opponent_action = current_opponent.pick_action(state, info)
            ...
            # Save model at the end of each generation
            is_generation_end = (ep + 1) % episodes_per_generation == 0
            # Also save at first episode and model_save_freq
            should_save = is_generation_end or ((ep + 1) % config.model_save_freq == 0) or (ep == 0)
            
            if should_save:
                dqn_agent.save_model(ep + 1)
                # If it's the end of a generation, add extra info
                if is_generation_end:
                    print(f"\n{'='*50}")
                    print(f"COMPLETED GENERATION {generation+1} at episode {ep+1}")
                    print(f"Model saved and ready for next generation's opponent")
                    print(f"{'='*50}\n")
...
class ModelAgent:
    """Agent that uses a saved DQN model to make decisions"""
    def __init__(self, model_path, state_size, action_size, device=None):
        if device is None:
            self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        else:
            self.device = device
        ...
        try:
            ...
            print(f"ModelAgent successfully loaded from {model_path}")
        except Exception as e:
            print(f"Error loading model from {model_path}: {e}")
            raise
        
    def pick_action(self, state, info):
        """Select action based on loaded model policy"""
        valid_actions = info["valid_actions"]
        valid_indices = np.where(valid_actions == 1)[0]
        
        # If no valid actions, return rest (should not happen)
        if len(valid_indices) == 0:
            return 0
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train DQN agent for Century game using self-play')
    ...
    config_dict = {
        ...
        'model_save_freq': args.model_save_freq if args.model_save_freq is not None else 50,  
        'gamma': args.gamma if args.gamma is not None else 0.99,  
        'epsilon': args.epsilon if args.epsilon is not None else 1.0,
        'epsilon_decay': args.epsilon_decay if args.epsilon_decay is not None else 0.985,  
        'epsilon_min': args.epsilon_min if args.epsilon_min is not None else 0.05,  
        'learning_rate': args.learning_rate if args.learning_rate is not None else 0.0007,  
        ...
        'num_timesteps': args.num_timesteps if args.num_timesteps is not None else 1000,
    }

    # Create config and start self-play DQN training
    config = DQNConfig(**config_dict)
    print("Starting self-play DQN training with generational progression")
    print("First generation vs RandomAgent, subsequent generations vs previous models")
