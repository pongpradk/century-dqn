...
class DQNAgent:
    ...
    def save_experience(self, state, action, reward, next_state, terminal):
        # Prioritize experiences with golem acquisition or high rewards
        priority = 1.0
        if reward > 5.0:  # Likely golem acquisition or win
            priority = 2.0
    ...
    try:
        for ep in range(start_episode, num_episodes):
            ...
            if ep % 500 == 0 and ep > 0:
                dqn_agent.epsilon = max(0.5, dqn_agent.epsilon + 0.2)
                print(f"Epsilon reset to {dqn_agent.epsilon} to encourage exploration")
