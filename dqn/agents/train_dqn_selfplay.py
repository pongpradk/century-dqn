...
Trains a DQN agent to play Century: Golem Edition YGBP using self-play.
"""
...
def _print_periodic_stats(agent, episode):
    if episode % 10 == 0:
        ...
        # Timestep stats
        if agent.timesteps_per_episode:
            recent_timesteps = agent.timesteps_per_episode[-10:]
            avg_timesteps = sum(recent_timesteps) / len(recent_timesteps)
            print(f"Avg timesteps per episode (last 10): {avg_timesteps:.1f}")
            print(f"Total timesteps so far: {agent.total_timesteps}")
